## Doing Stuff for
[Freifunk@Aalen](https://aalen.freifunk.net) | 
[Computertruhe e.V.](https://computertruhe.de) | 
[Hackwerk Aalen e.V.](https://aalen.space)

## Homepage
[njbraun | TIZEN](https://njbraun.de)***

[![CSS ist valide!](https://jigsaw.w3.org/css-validator/images/vcss)](https://jigsaw.w3.org/css-validator/validator?lang=de&profile=css3svg&uri=njbraun.de&usermedium=all&vextwarning=&warning=1) | [HTML ist valide!](https://validator.w3.org/nu/?doc=https%3A%2F%2Fnjbraun.de%2F) |
[Tested against Waves-A11ly](https://wave.webaim.org/report#/https://njbraun.de)


## Skill-Set
### Language
> only includes languages that I can* – languages that interest me are not listed here.
 - .md | MarkDown (common Use in Obsidian or things like this Page here)
 - .css | StyleSheet (Makes the Look of you WebPage)
 - .html | HyperTextMarkup-Language (Handels the Content of you Homepage)


### "IDE"/Tool-Set
- Adobe CC
- Affinity Serif (Ver.1/2)
- VSCode
- WordPress and Others
- Netcup, Hostnet Customerservice-Portal**

### Branch
> Buzzword for Crowd-Base Understanding
- Web/Photo-Design
- Printing and media technology*

### Other
- Windows (Vista - 10)
- Linux (Debian/Arch)

### Off-Code Skills
- Physical Security
- Photo/Movie*
- Drums, Guitar and Trashcan

## Making other Stuff
[Soundcloud](https://soundcloud.com/dertizen/lo-step-mp3?si=e4810754fee2496ab861f02da42f9ca1&utm_source=clipboard&utm_medium=text&utm_campaign=social_sharing)
[YouTube](https://www.youtube.com/watch?v=c8YDg4aXpx8&list=PL46VTLGy7yfZzjEu8kCliwoXuFwwQebqR&pp=gAQB0gcJCaIEOCosWNin)

## Internet-Gems
> These are external Links, please check before you blindly click on them. If one of them becomes sus/malicious pls. contact me.

[![JuneZone](https://thejunezone.neocities.org/JUNEBUTTON.gif)](https://thejunezone.neocities.org/) [![Pixeldesu](https://pixelde.su/assets/img/88x31/pixeldesu.png)](https://pixelde.su/) [![Fediring](https://fediring.net/static/badges/1.gif)](https://fediring.net)

## Contact
- [Mastodon | TIZEN](https://mastodon.social/@dertizen)

*_Basis: One-year vocational college with examination_

**_What you get, when you buy a Domain or Space by them. Can be Usefull for special quirks or Limitations_

*** WebPage getting a Re:Build soon
